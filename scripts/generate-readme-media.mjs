...
const THEMES = {
  light: {
    bg: "#fbfbf9", surface: "#ffffff", surface2: "#f3f3f0",
    fg: "#1a1a19", muted: "#6f6f68", border: "#e5e5df",
    accent: "#216e39", ring: "#30a14e",
    hm: ["#ebedf0", "#9be9a8", "#40c463", "#30a14e", "#216e39"],
    habit: { green: "#2f9e44", blue: "#1971c2", violet: "#6741d9", amber: "#e8590c", rose: "#d6336c", teal: "#0c8599" },
  },
  dark: {
    bg: "#0d1117", surface: "#161b22", surface2: "#1c2128",
    fg: "#e6edf3", muted: "#8b949e", border: "#2a313a",
    accent: "#2ea043", ring: "#39d353",
    hm: ["#161b22", "#0e4429", "#006d32", "#26a641", "#39d353"],
    habit: { green: "#4ade80", blue: "#60a5fa", violet: "#a78bfa", amber: "#fbbf24", rose: "#fb7185", teal: "#2dd4bf" },
  },
};
...
function text(x, y, s, { size = 12, fill, weight = 400, family = SANS, anchor = "start", spacing, opacity } = {}) {
  const attrs = [
...
const rect = (x, y, w, h, { fill = "none", rx = 0, stroke, sw = 1, opacity } = {}) =>
...
function banner(t) {
  ...
  const gx = Math.round((W - gridW) / 2), gy = 250;

  const cells = weeks.flatMap((col, w) =>
    col.map((lvl, d) =>
  ...
  const monthLabels = ["Oct", "Nov", "Dec", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep"]
    .map((m, i) => text(gx + Math.round(i * (gridW / 12)) + 1, gy - 12, m, { size: 11, fill: t.muted, family: MONO }))
    .join("");

  const legendX = gx + gridW - 118;
  const legendY = gy + 7 * step + 18;
  const legend =
    text(legendX - 8, legendY + 9, "Less", { size: 11, fill: t.muted, anchor: "end" }) +
    t.hm.map((c, i) => rect(legendX + i * 16, legendY, 11, 11, { fill: c, rx: 3, stroke: i === 0 ? t.border : undefined })).join("") +
    text(legendX + 5 * 16 + 4, legendY + 9, "More", { size: 11, fill: t.muted });

  let px = gx;
  const pills = ["Local-first", "Works offline", "No account needed", "MIT licensed"].map((p) => {
    const w = p.length * 7.1 + 28;
    const el = rect(px, 186, w, 30, { fill: t.surface, rx: 15, stroke: t.border }) +
...
for (const [mode, t] of Object.entries(THEMES)) write(`banner-${mode}.svg`, banner(t));
