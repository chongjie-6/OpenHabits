...
**A discovered creature idles; a silhouette does not** — coming alive is part of finding it. Each loop acts out its own blurb, so no two share a move: Sproutle's leaf is watered, Pebblit does everything twice, Mossback grows moss by keeping still and loses it when it moves. A creature's `rig` lifts boxes of its sprite out as parts and adds effect pixels; the `idle.css` beside the line's sprites animates them in whole sprite pixels held with `step-end`, so the art never leaves its grid. Effect pixels are hidden at rest, which is why the global reduced-motion rule, cutting every loop short, leaves each creature in its drawn pose. Because the stylesheet names parts, effects and colours by string, `tests/creatures.test.ts` reads it back and fails on any name the creature does not have — a typo there would otherwise animate nothing and say nothing. Tapping a discovered creature makes it hop, on a group of its own inside the sprite so the hop composes with whatever its idle loop is moving; a silhouette is not a button.
